...
namespace ft {
	...
	class set {
        public:
			...
			typedef typename allocator_type::template rebind<pair<const key_type ,value_type> >::other pair_allocator;
		...
		explicit set(const key_compare& comp = key_compare(),
                     const allocator_type& alloc = allocator_type()): _allocTree(tree_allocator(allocator_type())) {
		    _tree = _allocTree.allocate(1);
		    _allocTree.construct(_tree, tree(comp, pair_allocator(alloc)));
		};
		...
		set(InputIterator first, InputIterator last, const key_compare& comp = key_compare(),
                            ...
                            _allocTree(tree_allocator(allocator_type())) {
                _tree = _allocTree.allocate(1);
                _allocTree.construct(_tree, tree(first, last, comp, pair_allocator(alloc)));
            };

			// copy
		// Constructs a container with a copy of each of the elements in x.
		set(const set& copy): _allocTree(copy._allocTree)  {
		    _tree = _allocTree.allocate(1);
		    _allocTree.construct(_tree, *(copy._tree));
		};

		// Operators ----
		set &operator=(const set& copy) {
			if (_tree->getTree() != copy._tree->getTree()) {
				if (!_tree->empty()) {
					_tree->clear();
					_allocTree.destroy(_tree);
				}
				_allocTree = copy._allocTree;
				_allocTree.construct(_tree, *(copy._tree));
			}
		    return *this;
		};
