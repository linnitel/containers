...
namespace ft {
	...
	public:
		...
		explicit map(const key_compare& comp = key_compare(),
		    ...
		    _allocTree.construct(_tree, tree(comp, alloc));
		};
		...
		map(InputIterator first, InputIterator last, const key_compare& comp = key_compare(),
            ...
            _tree = _allocTree.allocate(1);
            _allocTree.construct(_tree, tree(first, last, comp, alloc));
        };
			// copy
		// Constructs a container with a copy of each of the elements in x.
		map(const map& copy): _allocTree(copy._allocTree) {
		    _tree = _allocTree.allocate(1);
		...
		map &operator=(const map &copy) {
		    if (_tree->getTree() != copy._tree->getTree()) { // comparison should be of all items of the tree
		        if (!_tree->empty()) {
		            _tree->clear();
		            _allocTree.destroy(_tree);
		        }
